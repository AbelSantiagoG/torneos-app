...
export async function generarFixtureTodosContraTodosFase(params: {
  ...
  const { error } = await supabase.rpc('generar_fixture_todos_contra_todos_fase', {
    p_fase_torneo_id: params.faseTorneoId,
    p_ida_vuelta: Boolean(params.idaVuelta),
  })
  if (error) throw toUserError(error, 'fixture')
}

export async function generarFixtureEliminatoriaDirectaFase(params: {
  faseTorneoId: string
  idaVuelta?: boolean
  tipoCruce: string
}): Promise<void> {
  const { error } = await supabase.rpc('generar_fixture_eliminatoria_directa_fase', {
    p_fase_torneo_id: params.faseTorneoId,
    p_ida_vuelta: Boolean(params.idaVuelta),
    p_tipo_cruce: params.tipoCruce,
  })
  if (error) throw toUserError(error, 'fixture')
}

export async function generarLlavesEliminatoriaSinEquipos(params: {
  faseTorneoId: string
  cantidadEquipos: number
  idaVuelta?: boolean
}): Promise<void> {
  const { error } = await supabase.rpc('generar_llaves_eliminatoria_sin_equipos', {
    p_fase_torneo_id: params.faseTorneoId,
    p_cantidad_equipos: params.cantidadEquipos,
    p_ida_vuelta: Boolean(params.idaVuelta),
  })
  if (error) throw toUserError(error, 'fixture')
}
